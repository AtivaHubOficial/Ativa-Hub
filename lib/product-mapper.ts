...
export type ProductRow = {
  ...
  specifications: Array<{ name: string; value: string }> | null; created_at: string;
};
...
export function rowToProduct(row: ProductRow): Product {
  return {
    ...
    imageUrl: row.image_url ?? "", gallery: row.gallery ?? [], description: row.description ?? "",
    shortDescription: row.short_description ?? "", features: row.features ?? [], tags: row.tags ?? [], affiliateUrl: row.affiliate_url,
    status: row.status, featured: Boolean(row.featured), specifications: row.specifications ?? [], createdAt: row.created_at,
  };
}
