...
export async function resolveCatalogProduct<TItem>(
  ...
  request:<T>(path:string,optional404?:boolean)=>Promise<T|undefined>,
):Promise<CatalogResolution<TItem>>{
  const product=await request<CatalogProduct>(`/products/${encodeURIComponent(productId)}`);
  let itemId=preferredItemId;
  let offer:CatalogOfferSummary|undefined;
  if(!itemId){
    const offers=await request<CatalogOffers>(`/products/${encodeURIComponent(productId)}/items`,true);
  ...
  const item=itemId?await request<TItem>(`/items/${encodeURIComponent(itemId)}`,true):undefined;
