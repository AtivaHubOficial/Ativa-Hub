        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {partners.map((partner) => (
            <article key={partner.key} className="flex flex-col rounded-2xl border bg-white p-5 shadow-sm">
              ...
              {partner.actions.length ? <div className="mt-4 flex flex-wrap gap-2">{partner.actions.map((action) => <Link key={action.href} href={action.href} className="inline-flex min-h-10 items-center gap-1 rounded-xl border px-3 text-sm font-bold">{action.label}<ExternalLink size={14} /></Link>)}{partner.key==="mercado_livre"&&partner.accountId?<><Link href="/api/auth/mercado-livre/start" className="inline-flex min-h-10 items-center rounded-xl border px-3 text-sm font-bold">Renovar conexão</Link><button onClick={disconnect} className="min-h-10 rounded-xl border border-red-200 px-3 text-sm font-bold text-red-700">Desconectar</button></>:null}</div> : <p className="mt-4 text-sm font-bold text-slate-400">Nenhuma ação disponível.</p>}
            </article>
          ))}
        </div>
      )}
