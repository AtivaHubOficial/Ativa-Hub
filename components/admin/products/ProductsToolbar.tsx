...
import type { FeaturedFilter, SourceFilter, StatusFilter } from "@/components/admin/products/product-filters";

interface ProductsToolbarProps {
  search: string; category: string; status: StatusFilter; featured: FeaturedFilter; source: SourceFilter; categories: string[]; hasFilters: boolean;
  onSearchChange: (value: string) => void; onCategoryChange: (value: string) => void; onStatusChange: (value: StatusFilter) => void; onFeaturedChange: (value: FeaturedFilter) => void; onSourceChange: (value: SourceFilter) => void; onClear: () => void;
}
...
export default function ProductsToolbar(props: ProductsToolbarProps) {
  return (
    <div className="grid gap-3 lg:grid-cols-[minmax(230px,1fr)_repeat(4,minmax(140px,auto))_auto]">
      ...
      <label className="grid gap-1"><span className="sr-only">Filtrar por origem</span><select value={props.source} onChange={(event) => props.onSourceChange(event.target.value as SourceFilter)} className={selectClassName}><option value="all">Todas as origens</option><option value="manual">Manual</option><option value="logzz">Logzz</option><option value="mercado_livre">Mercado Livre</option></select></label>
